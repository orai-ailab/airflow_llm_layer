...
def process_data_and_save():
    try:
        ...
        coin_dataframe['market_cap_dominant'] = coin_dataframe['market_cap'] / ( coin_dataframe['market_cap'].sum())
        coin_dataframe['updated_at'] = datetime.now()
        coin_dataframe.drop(['price_change_percentage_24h_in_currency', 'roi', 'max_supply'], axis=1, inplace=True)
